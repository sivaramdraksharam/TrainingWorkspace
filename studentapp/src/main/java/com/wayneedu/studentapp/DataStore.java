...
public class DataStore {
		...
		try (BufferedReader br = new BufferedReader(new FileReader(file))) {
			...
			while ((line = br.readLine()) != null) {
				if (i > 0) {
					...
					Date dob = dateFormat.parse(values[2]);
			...
			throw new ApplicationStartException(
					"Error Starting application. CSV File not found");
		} catch (IOException e) {
			throw new ApplicationStartException(
					"Error Starting application.Invalid  CSV File");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void deleteStudentDetail(String id) {
		...
		System.out.println("Student with id :" + id
				+ " removed Successfully from datastore");
	}
