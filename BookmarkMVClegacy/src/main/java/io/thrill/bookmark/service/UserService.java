...
public class UserService {
	...
	public boolean saveUserBookmark(Long userId, Long bookmarkId) {
		...
		if (user != null) {
			System.out.println("check"+user.getBookmarkList().contains(bookmark));
			boolean isExit = user.getBookmarkList().contains(bookmark);
			if(!isExit){
				user.addBookmark(bookmark);
			}
			
			
			//System.out.println("result: " + userDao.saveUserBookmark(user));
			status = true;
		}
		return status;  
	}
